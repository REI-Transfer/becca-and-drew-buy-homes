 ...
 *
 * Flat ordered 8-chapter sequence (no thematic categories). The player is a
 * Blob-hosted native <video> (not an iframe). All brand color comes from the
 * `accentColor` prop (NOT hardcoded).
 *
 * NOTE: do NOT import "@/lib/config" here -- this is a client component. Branding
 * is passed in as props from the server page.
 *
 * encodeSegment + buildSrc + the autoplay/tap-for-sound player logic are ported
 * byte-for-byte from jenkins-homebuyers; only the chapter list is flat (no accordion).
 */
...
export function VideoGallery({ accentColor }: VideoGalleryProps) {
  ...
  const selectedIndex = playable.findIndex((v) => v.src === selected.src)
  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row lg:gap-6">
        ...
        <div className="overflow-hidden rounded-2xl bg-black shadow-2xl">
          ...
          <div className="bg-gray-900 px-5 py-3">
            <span
              ...
              style={{ backgroundColor: accentColor }}
            >
              Chapter {selectedIndex + 1} of {playable.length}
            </span>
      ...
      {/* Chapters sidebar -- flat ordered list (no categories) */}
